...
uint32_t nrf_pwm_init(nrf_pwm_config_t * config)
{
    ...
    switch (config->mode) {
        ...
        // 0-100 resolution, 321Hz PWM freq, 32kHz timer freq (prescaler 9)
        case PWM_MODE_LED_100:   
            PWM_TIMER->PRESCALER = 9; 
            pwm_max_value = 100;
            break;

        // 8-bit resolution, 122Hz PWM freq, 32kHz timer freq (prescaler 9)
        case PWM_MODE_LED_255:   
            PWM_TIMER->PRESCALER = 9;
            pwm_max_value = 255;  
            break;

        // 0-1000 resolution, 250Hz PWM freq, 250kHz timer freq (prescaler 6)
        case PWM_MODE_LED_1000:  
            PWM_TIMER->PRESCALER = 6;
            pwm_max_value = 1000;
            break;

        // 0-100 resolution, 20kHz PWM freq, 2MHz timer freq (prescaler 3)
        case PWM_MODE_MTR_100:   
            PWM_TIMER->PRESCALER = 3;
            pwm_max_value = 100;
            break;

        // 8-bit resolution, 31kHz PWM freq, 8MHz timer freq (prescaler 1)  
